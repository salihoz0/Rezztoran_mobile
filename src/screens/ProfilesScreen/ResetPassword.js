...
import { usePostResetUserPassword, usePostResetPasswordCode } from '../../api/auth';
    ...
    return (
        <SafeAreaView edges={['bottom']} style={{ backgroundColor: '#FFFFFF', flex: 1 }}>
            ...
            <View style={{ borderWidth: 1 }}>
                <Text>mailinizi giriniz</Text>
                <CustomInput placeholder="e-posta" value={mail} setValue={setMail} />
                <CustomButton text='mail gönder' onPress={() => doResPassword(mail)} />
            </View>
            {
                !isModalVisible && (
                    <View style={{ borderWidth: 1, backgroundColor: '#FFFFFF' }}>
                        <Text>Kodu Gir</Text>
                        <CustomInput placeholder="6 Haneli Kod" value={code} setValue={setCode} />
                        <Text>Yeni Şifre Gir</Text>
                        <CustomInput placeholder="Yeni Şifre" value={password} setValue={setPassword} />
                        <View style={{ flexDirection: 'row', justifyContent: 'space-evenly' }}>
                            <CustomButton
                                text='Kodu Gönder'
                                onPress={() => doSendNewCodeAndPassword({ mail: mail, password: password, code: code })}
                                type='SMALL'
                                bgColor="#FFFFFF"
