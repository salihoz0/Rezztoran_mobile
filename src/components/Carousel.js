...
const Carousel = (props) => {
    ...
    const renderItem = ({ item }) => {
        const { restaurantImage, restaurantName, city, price, starCount, id } = item
        return (
            <View
                style={{
                    marginHorizontal: 5,
                ...
                <TouchableOpacity style={{ position: 'absolute', right: 10, alignItems: 'center', marginTop: 10, zIndex: 2 }} onPress={() => { handleFavoriteButtonPress(id, restaurantName, city, price, restaurantImage) }}>
                    {
                ...
                <View style={{ marginHorizontal: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ fontSize: 15, fontFamily: 'Poppins-Medium', color: 'black', marginTop: 5, width: 130 }}>{restaurantName}</Text>
                ...
                <View style={{ marginHorizontal: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, marginVertical: 10 }}>
                    ...
                    <Text style={{ paddingHorizontal: 10, alignItems: 'center', justifyContent: 'space-between', color: '#000000' }}> <Icon name="star" size={15} color={'rgb(212, 123, 51)'} /> {starCount}</Text>
                </View>
            </View>
        );
    }
